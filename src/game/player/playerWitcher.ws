...
{	
	...
	//modW3ReduxRGC++
	private var rgcSignQuickUseModifierActive : bool;
		default rgcSignQuickUseModifierActive = false;
		
	private saved var rgcLastUsedItemSlot : EEquipmentSlots;
		default rgcLastUsedItemSlot = EES_Quickslot1;

	private saved var rgcLastUsedBolt : SItemUniqueId;
		default rgcLastUsedBolt = GetInvalidUniqueId();

	private var rgcInfiniteBoltName : name;
		default rgcInfiniteBoltName = 'Bodkin Bolt';
	//modW3ReduxRGC--
	...
	//modW3ReduxRGC++
	public function RGC_Update() : void
	{
		var boltsArray : array<SItemUniqueId>;
		var equippedBolt, newInfiniteBolt, oldInfiniteBolt : SItemUniqueId;
		var currentInfiniteBoltName : name;
		var i : int;
		
		GetItemEquippedOnSlot(EES_Bolt, equippedBolt);
		currentInfiniteBoltName = GetCurrentInfiniteBoltName();

		if(currentInfiniteBoltName != rgcInfiniteBoltName) 
		{
			boltsArray = inv.AddAnItem(currentInfiniteBoltName, 1, true, true);
			newInfiniteBolt = boltsArray[0];
			boltsArray = inv.GetItemsByName(rgcInfiniteBoltName);
			if(boltsArray.Size() > 0)
			{
				oldInfiniteBolt = boltsArray[0];
				if(equippedBolt == oldInfiniteBolt) {
					RGC_InfiniteBolt(newInfiniteBolt);
				}

				for(i=boltsArray.Size()-1; i>=0; i-=1)
					inv.RemoveItem(boltsArray[i], inv.GetItemQuantity(boltsArray[i]) );
			}

			rgcInfiniteBoltName = currentInfiniteBoltName;
		}
	...
	//modW3ReduxRGC++
	public function RGC_SelectItem( nonCrossbowSlot : EEquipmentSlots)
	{
		//This function is meant to select non crossbow items
		//RGC has seperated the crossbow from all other usable items
		var newEquipItem		: SItemUniqueId;
		var currentItem			: SItemUniqueId;
		var playSound			: bool;
		var soundName			: string;
		
		playSound = false;
		if(nonCrossbowSlot == EES_RangedWeapon)
		{
			return;
		}
		if(rgcLastUsedItemSlot == nonCrossbowSlot)
		{
			newEquipItem = RGC_NextItemForSlot(nonCrossbowSlot);
			if(GetInventory().IsIdValid(newEquipItem)) 
			{
				EquipItem(RGC_NextItemForSlot(nonCrossbowSlot), nonCrossbowSlot);
				playSound = true;
			}
		} 
		else
		...
		if(GetItemEquippedOnSlot(nonCrossbowSlot, currentItem))
		{
			if(GetInventory().IsItemBomb(currentItem))
			{
				if(GetInventory().SingletonItemGetAmmo(currentItem))
				{
					SelectQuickslotItem(nonCrossbowSlot);
					soundName = "gui_inventory_bombs_attach";
				}
			}
			else
			{
				SelectQuickslotItem(nonCrossbowSlot);
				soundName = "gui_inventory_other_attach";
			}
		}
		...
		rgcLastUsedItemSlot = nonCrossbowSlot;
	}
	
	public function RGC_GetLastUsedItemSlot() : EEquipmentSlots
	{
		return rgcLastUsedItemSlot;
	}

	public function RGC_GetLastUsedBolt() : SItemUniqueId
	{
		return rgcLastUsedBolt;
	}

	public function RGC_GetInfiniteBolt() : SItemUniqueId
	{
		var items : array<SItemUniqueId>;

		items = GetInventory().GetItemsByName(rgcInfiniteBoltName);
		return items[0];
	}

	public function RGC_NextItemForSlot(slot: EEquipmentSlots) : SItemUniqueId
	{
		var i 					: int;
		var iterableItem		: SItemUniqueId;
		var equippedItem		: SItemUniqueId;
		var dogReportItem		: SItemUniqueId;
		var itemsListUnfiltered	: array<SItemUniqueId>;
		var itemsList			: array<SItemUniqueId>;
		var numItems			: int;
		var itemCategory		: CName;
		
		dogReportItem = GetInvalidUniqueId();

		if(slot == EES_Petard1) 
		{
			GetItemEquippedOnSlot(slot, equippedItem);
			itemCategory = 'petard';
		}
		else if(slot == EES_Quickslot1)
		{
			GetItemEquippedOnSlot(slot, equippedItem);
			itemCategory = 'usable';
		}
		...
		{
			equippedItem = rgcLastUsedBolt;
			itemCategory = 'bolt';
		}
		else
		{
			return dogReportItem;
		}
		
		itemsListUnfiltered = inv.GetItemsByCategory(itemCategory);

		for ( i = 0; i < itemsListUnfiltered.Size(); i += 1 )
		{
			iterableItem = itemsListUnfiltered[i];
			if ( 
				inv.GetItemLevel( iterableItem ) <= GetLevel() && 
				(!inv.IsItemSingletonItem(iterableItem) || inv.SingletonItemGetAmmo(iterableItem) || iterableItem == equippedItem) 
				&& !inv.ItemHasTag(iterableItem, theGame.params.TAG_INFINITE_AMMO) )
			{
				itemsList.PushBack(iterableItem);
			}
		}

		numItems = itemsList.Size();
		if (numItems >= 1) {
			for ( i = 0; i < numItems; i += 1 )
			{
				iterableItem = itemsList[i];
				
				if(iterableItem == equippedItem || equippedItem == GetInvalidUniqueId() || !GetInventory().HasItemById(equippedItem))
				{
					if(i != numItems - 1)
					{
						dogReportItem = itemsList[i+1];
					}
					else
					{
						dogReportItem = itemsList[0];
					}
					break;
				}
			}
	...
	public function RGC_InfiniteBoltEquipped() : bool
	{
		var currentBolt : SItemUniqueId;

		GetItemEquippedOnSlot(EES_Bolt, currentBolt);
		return GetInventory().ItemHasTag(currentBolt, theGame.params.TAG_INFINITE_AMMO);
	}
	
	public function RGC_NextBolt()
	{
		var currentBolt, newBolt : SItemUniqueId;
		...
		GetItemEquippedOnSlot(EES_Bolt, currentBolt);
		newBolt = rgcLastUsedBolt;
		if(!RGC_InfiniteBoltEquipped()) 
		{
			newBolt = RGC_NextItemForSlot(EES_Bolt);
		}

		if(newBolt != currentBolt && newBolt != GetInvalidUniqueId())
		{
			EquipItem(newBolt, EES_Bolt);
			SoundEvent("gui_inventory_other_attach");
			rgcLastUsedBolt = newBolt;
		}
	}

	public function RGC_InfiniteBolt(optional newBolts: SItemUniqueId)
	{
		var infiniteBolts : array<SItemUniqueId>;

		if(GetInventory().IsItemBolt(newBolts) && GetInventory().ItemHasTag(newBolts, theGame.params.TAG_INFINITE_AMMO))
		{
			EquipItem(newBolts, EES_Bolt);
			SoundEvent("gui_inventory_other_attach");
		}
		else if(!RGC_InfiniteBoltEquipped()) 
		{
			infiniteBolts = GetInventory().GetItemsByName(rgcInfiniteBoltName);
			EquipItem(infiniteBolts[0], EES_Bolt);
			SoundEvent("gui_inventory_other_attach");
		}
	}
	
	public function RGC_SetInRealtimeEquipCastMode( modifierActive : bool )
	{
		rgcSignQuickUseModifierActive = modifierActive;
	}
	
	public function RGC_IsInRealtimeEquipCastMode() : bool
	{
		return rgcSignQuickUseModifierActive;
	}
	//modW3ReduxRGC--
	...
	public final function FlipSelectedPotion(isUpperSlot : bool) : bool
	{
		...
		if(isUpperSlot)
		{
			...
			{
				SoundEvent("gui_inventory_potion_attach");
			...
			{	if ( !IsSwimming() || !IsDiving() )
				{
					...
					//rangedWeapon.OnWeaponReload(); //modW3ReduxRGC
				}
				else
				{
