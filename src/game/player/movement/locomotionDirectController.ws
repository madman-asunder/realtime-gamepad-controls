...
/***********************************************************************/
/** 	Immersive Motion for Gamepads - Re Envisioned!  v 1.0
/***********************************************************************/
...
class CR4LocomotionPlayerControllerScript extends CR4LocomotionDirectControllerScript
{
	...
	var nextTime				: float; //used for debugging
	//ImmersiveMotion--
	...
	function UpdateLocomotion()
	{
		...
		// ImmersiveMotion - moved the var, previousSpeed out of UpdateLocomotion to make it accessible to the IM slow start functions
		var inputInCameraSpace, inputAngleToRotate, haxStrafe, haxForward : float;
		var playerYaw				: float;
		...
		{					
			...
			else
			{
				...
				{
					moveSpeed	= MinF( moveSpeed, speedSlowWalkingMax ); //Immersive Motion
				}
			}
		}
		else
		...
		player.SetIsWalking( moveSpeed > 0.01f ); //modW3ReduxRGC
		player.SetIsRunning( moveSpeed > 0.80f ); //modW3ReduxRGC
	}
	...
	function CalculateMoveSpeed() : float
	{
		...
		if( RGCConfigIsImmersiveMotionEnabled() ) {
			return ImmersivelyCalculateMoveSpeed();
		}
		//modW3ReduxRGC++
	...
	function ImmersivelyCalculateMoveSpeed() : float
	{
		var speedVec 		: Vector;
		var speed 			: float;
		var rawRightJoyVec	: Vector;
		...
		var leftStickLength 			: float;
		var slowStart 					: bool;

		slowStart = false;
		leftStickLength = _inputMagCurr;

		if( leftStickLength > 1.0 ) leftStickLength = 1.0;
		...
		{
			//ImmersiveMotion++
			speed = leftStickLength * speedWalkingMax;
			//ImmersiveMotion--
		}
		else
		{
		...
		if( thePlayer.IsSprintActionPressed() && speed > 0) //ImmersiveMotion
		{
			if ( thePlayer.IsInCombat()
				&& thePlayer.moveTarget
				&& VecDistance( thePlayer.moveTarget.GetWorldPosition(), thePlayer.GetWorldPosition() ) < thePlayer.findMoveTargetDistMax )
		...
		else if ( !player.disableSprintingTimerEnabled && player.GetIsSprinting() )
		{
			player.disableSprintingTimerEnabled = true;
			player.AddTimer( 'DisableSprintingTimer', 0.25f );
		}
		...
		else
		{
			...
			{
				thePlayer.SetWalkToggle(false);
				//thePlayer.SetSprintToggle(false); //RGC
			}
			...
			else if (!theGame.IsFading() && !theGame.IsBlackscreen())
			{
				if ( player.GetIsSprinting() )
				{
					if(player.IsInCombat()) {
						...
						speed = player.rgcSprintSpeed;
					}

					speed = (leftStickLength * (speed - 0.8f)) + 0.8f;
				...
				else if( player.rgcRunningLanding != 0)
				{
					speed = player.rgcRunningLanding;
					if( speed > 1.2 )
						player.playerMoveType = PMT_Sprint;
					else
						player.playerMoveType = PMT_Run;
				...
				else if ( player.IsInCombat() )
				{
					if ( player.IsOnBoat() )
					{
						speed = speedSlowWalkingMax;
						player.playerMoveType = PMT_Walk;
					}
					else if ( leftStickLength >= 0.8f )
					{
						speed = speedWalkingMax;
						player.playerMoveType = PMT_Walk;
					}
		...
		/* Immersive Motion Debug
		currentTime = EngineTimeToFloat( theGame.GetEngineTime() );
		if( currentTime > nextTime )
		{
			nextTime = currentTime + 6.0;
			thePlayer.DisplayHudMessage( FloatToString( speed ) );
		}
		*/

		// ImmersiveMotion++
		if( thePlayer.GetBehaviorVariable( 'inJumpState' ) == 1.f && speed > 0.85 )
		{
			if( speed > 1.2 )
				thePlayer.RgcEnableRunningLanding( 1.2, 1.1 );
			else
				thePlayer.RgcEnableRunningLanding( 0.9, 0.9 );
		}
		if( speed > 0 && previousSpeed == 0 && thePlayer.GetCurrentStateName() == 'Exploration' && slowStart )
		{
			thePlayer.RgcStartSlow();
		}
		// ImmersiveMotion--
