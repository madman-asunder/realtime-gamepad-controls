...
{ 
	...
	protected function ProcessSelectionWeights() : STargetSelectionWeights
	{
		...
		{
			//modW3ReduxRGC++
			if ( theInput.GetActionValue( 'RGC_CrossBow' ) == 1.f || ( rangedWeapon && rangedWeapon.GetCurrentStateName() != 'State_WeaponWait' ) )
			{
			//modW3ReduxRGC--
				selectionWeights.angleWeight = 1.f;
				selectionWeights.distanceWeight = 0.f;
		...
		switch ( actionToRestore )
		{
			...
			case PATR_Crossbow:
				if ( inv.IsItemCrossbow ( selectedItemId ) )
				{
					...
					//modW3ReduxRGC++
					if ( theInput.IsActionPressed( 'RGC_CrossBow' ) )
					{
					//modW3ReduxRGC--
	...
	public function CreateInput()
	{
		...
		inputHandler = new W3ReduxRGCInput in this;
		//modW3ReduxRGC--
	...
	// Immersive Motion (RGC)++
	public			var rgcRunningLanding	: float;
	public			var rgcSprintSpeed		: float;

	private timer function RgcDeactivateRunningLanding(dt: float, id: int)
	{
		rgcRunningLanding = 0;
	}

	public function RgcEnableRunningLanding(rlSpeed : float, time : float)
	{
		if( rgcRunningLanding == 0)
		{
			rgcRunningLanding = rlSpeed;
			AddTimer('RgcDeactivateRunningLanding', time, false);
		}
	}

	var rgcSlowStartID		: int;	default rgcSlowStartID = -1;
	var rgcSlowStartActive	: bool;

	private timer function RgcRemoveStartSlow( dt: float, id: int )
	{
		ResetAnimationSpeedMultiplier(rgcSlowStartID);
		rgcSlowStartID = SetAnimationSpeedMultiplier(0.9);
		AddTimer( 'RgcRemoveStartSlow2', 1.0, false );
	}

	private timer function RgcRemoveStartSlow2( dt: float, id: int )
	{
		ResetAnimationSpeedMultiplier(rgcSlowStartID);
		rgcSlowStartActive = false;
	}

	public function RgcStartSlow()
	{
		if( !rgcSlowStartActive )
		{
			rgcSlowStartActive = true;
			rgcSlowStartID = SetAnimationSpeedMultiplier(0.8);
			AddTimer( 'RgcRemoveStartSlow', 1.0, false );
		}
	}

	public function RgcSetSprintSpeed( speed : float )
	{
		rgcSprintSpeed = speed;
	}
	// Immersive Motion--
}
