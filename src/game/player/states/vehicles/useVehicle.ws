...
statemachine class W3VehicleCombatManager extends CEntity
{
	...
	public function Setup( player : CR4Player, _vehicle : CVehicleComponent )
	{
		...
		if( rider )
		{
			...
			//modW3ReduxRGC++
			theInput.RegisterListener( this, 'OnRGCHorseCastAxii', 'RGC_HorseCastAxii' );
			theInput.RegisterListener( this, 'OnItemAction', 'RGC_HorseCrossBow' );
			theInput.RegisterListener( this, 'OnItemActionHold', 'RGC_HorseCrossBowHold' );
			//modW3ReduxRGC--
		}
	}
...
state Null in W3VehicleCombatManager
{
	...
	entry function ShouldEnterNextState()
	{
		...
		else if ( theInput.IsActionPressed('RGC_HorseCrossBow') )
		{
			StartCrossBowAction();
		}
		else if ( theInput.IsActionPressed('RGC_HorseCastAxii') )
		//modW3ReduxRGC--
		{
			StartCastSignAction();
		}
	...
	{	
		...
		{
			//modW3ReduxRGC++
			if(theInput.IsActionPressed('VehicleItemAction'))
			{
				StartItemAction();
			}
			else if(theInput.IsActionPressed('RGC_HorseCrossBow'))
	...
	//modW3ReduxRGC++
	event OnRGCHorseCastAxii( action : SInputAction )
	{
		if ( horseComp && !horseComp.IsFullyMounted() )
		{
			parent.GotoState('MountingInProgress');
		...
		if ( !GetWitcherPlayer().RGC_IsInRealtimeEquipCastMode() )
		{
			//only cast axii if in realtime cast mode
			return false;
		}
	...
	function StartItemAction()
	{
		...
		{
			if ( thePlayer.IsHoldingItemInLHand() ) thePlayer.HideUsableItem(true);
	...
	//modW3ReduxRGC++
	function StartCrossBowAction()
	{
		if ( thePlayer.IsHoldingItemInLHand() ) thePlayer.HideUsableItem(true);
		GetWitcherPlayer().SelectQuickslotItem(EES_RangedWeapon);
		if( rider.GetInventory().IsItemCrossbow( rider.GetSelectedItemId() ) && rider.IsActionAllowed( EIAB_Crossbow ) )
		{
			rider.SetBehaviorVariable( 'actionType', (int)EHCA_ShootCrossbow );
			rider.rangedWeapon = ( Crossbow )( rider.GetInventory().GetItemEntityUnsafe( rider.GetSelectedItemId() ) );
			rider.rangedWeapon.OnRangedWeaponPress();
...
state CastSign in W3VehicleCombatManager
{
	...
	entry function StartCastingSign()
	{
		...
		while( true )
		{
			...
			//modW3ReduxRGC++
			if( theInput.IsActionPressed( 'RGC_HorseCastAxii' )  )
			//modW3ReduxRGC--
			{
	...
	event OnLeaveState( nexStateName : name )
	{
		...
		//modW3ReduxRGC++
		theInput.ForceDeactivateAction( 'RGC_HorseCastAxii' );
		//modW3ReduxRGC--
...
state ThrowProjectile in W3VehicleCombatManager extends RangedAttack 
{
	...
	{	
		//modW3ReduxRGC++
		if(action.aName == 'VehicleItemAction')
		{
			if ( thrownEntity )
	...
	{
		//modW3ReduxRGC++
		if(action.aName == 'VehicleItemActionHold')
		{
			if( IsPressed( action ) )
			{
				ItemActionHold();
...
state ThrowBomb in W3VehicleCombatManager extends ThrowProjectile
{
	event OnEnterState( prevStateName : name )
	{
		...
		rider.SetThrowHold(true); //modW3ReduxRGC
	}
...
state ShootCrossbow in W3VehicleCombatManager extends RangedAttack
{
	event OnEnterState( prevStateName : name )
	{
		...
		//modW3ReduxRGC++
		if ( theInput.IsActionPressed('RGC_HorseCrossBowHold') )
		{
			ItemActionHold();
		}

		rider.SetIsAimingCrossbow(true);
		//modW3ReduxRGC--
	}
	...
	{
		super.OnLeaveState(nextStateName);
		rider.SetIsAimingCrossbow(false);
	}
	//modW3ReduxRGC--
	...
	{
		//modW3ReduxRGC++
		if(action.aName == 'RGC_HorseCrossBow')
		{
			super.OnItemAction( action );
	...
	{
		//modW3ReduxRGC++
		if(action.aName == 'RGC_HorseCrossBowHold')
		{
			...
			{
				theInput.ForceDeactivateAction('RGC_HorseCrossBow');
			}
		}
		//modW3ReduxRGC--
	}
