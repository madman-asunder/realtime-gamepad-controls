...
{		
	...
	{	
		var conf : int;
		var prevInputHandler : CPlayerInput; //modW3ReduxRGC
		...
		if( prevInputHandler )
		{
			inputHandler.Initialize(spawnData.restored, prevInputHandler);
		}
		else
		{
	...
	protected function ShouldDrainStaminaWhileSprinting() : bool
	{
		...
		if ( sprintingTime >= 3.0 || GetStaminaPercents() < 1.0 )
		{
			...
			if( currentStateName == 'Exploration' || currentStateName == 'CombatSteel' || currentStateName == 'CombatSilver' || currentStateName == 'CombatFists' )
			{
				...
				//modW3ReduxRGC++
				if( RGCConfigGetSprintStaminaUsage() == ESSU_Never )
				{
					return false;
				}
				else if(RGCConfigGetSprintStaminaUsage() == ESSU_Combat)
				{
					return currentStateName == 'CombatSteel' || currentStateName == 'CombatSilver' || currentStateName == 'CombatFists';
				}
				else
				{
					return currentStateName == 'Exploration' || currentStateName == 'CombatSteel' || currentStateName == 'CombatSilver' || currentStateName == 'CombatFists';
				}
				//modW3ReduxRGC--
			}
		}
	...
	timer function DisableSprintingTimer ( time : float , id : int)
	{
		...
		if ( !thePlayer.CanSprint( VecLength( rawLeftJoyVec ) ) )
		{
			...
			thePlayer.SetSprintToggle( false ); //modW3ReduxRGC
