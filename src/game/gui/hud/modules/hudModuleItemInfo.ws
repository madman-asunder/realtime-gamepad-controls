...
class CR4HudModuleItemInfo extends CR4HudModuleBase
{
	...
	event OnTick( timeDelta : float )
	{
		...
		else
		{
			...
			//modW3ReduxRGC++
			if(!witcherPlayer.GetIsAimingCrossbow())
			{
				witcherPlayer.GetItemEquippedOnSlot( witcherPlayer.RGC_GetLastUsedItemSlot(), item0 );
			}
			...
			if ( theInput.LastUsedGamepad() )
			{
				...
				{
					item1 = witcherPlayer.RGC_GetLastUsedBolt();
					item2 = witcherPlayer.RGC_GetInfiniteBolt();
					alterItem1 = witcherPlayer.RGC_NextItemForSlot(EES_Bolt);
				}
				else if(RGC_IsQuickItemsInventory())
				{
					witcherPlayer.GetItemEquippedOnSlot( EES_Quickslot1, item1 );
					witcherPlayer.GetItemEquippedOnSlot( EES_Petard1, item2 );
					alterItem1 = witcherPlayer.RGC_NextItemForSlot(EES_Quickslot1);
					alterItem2 = witcherPlayer.RGC_NextItemForSlot(EES_Petard1);
				
					if ( !playerInv.IsIdValid(item1) && playerInv.IsIdValid( alterItem1 ) )
					{
						witcherPlayer.RGC_SelectItem(EES_Quickslot1);
					}
					else if ( !playerInv.IsIdValid(item2) && playerInv.IsIdValid( alterItem2 ) )
					{
						witcherPlayer.RGC_SelectItem(EES_Petard1);
					}
					else
					{
						switchAnimation = -1;
		...
		if ( thePlayer.IsCombatMusicEnabled() || thePlayer.GetHealthPercents() < 1.f )
			...
			SetAlwaysDisplayed( RGCConfigAlwaysDisplayInventory() ); //modW3ReduxRGC

		//modW3ReduxRGC++
		tickCount += 1;
		if(tickCount == 10)
		{
			if(RGCConfigAlwaysDisplayInventory())
			{
				ForceShowElement();
			}
		...
		if ( thePlayer.GetInventory().IsIdValid( currItem ) )
		{
			if ( thePlayer.GetInventory().IsItemSingletonItem( currItem ) )
			{
				...
				if ( prevItemAmmo != ammo )
				{
					...
					if(!ammo)
					{
						if(thePlayer.inv.IsItemBomb( currItem ))
						{
							RGC_ActOnBackupBomb();
						}
						else if(thePlayer.inv.IsItemPotion( currItem ))
						{
							RGC_ActOnBackupPotion( currItem );
						}
					}
					//modW3ReduxRGC--
					updateItem = true;
			...
			else if( thePlayer.GetInventory().ItemHasTag( currItem, 'Edibles' ) || thePlayer.GetInventory().IsItemBolt( currItem ) )
			{				
			...
			else if( thePlayer.GetInventory().IsItemBolt( currItem ))
			{
				ammo = thePlayer.GetInventory().GetItemQuantity( currItem );
				if ( prevItemAmmo != ammo )
				{
			...
			//modW3ReduxRGC++
			else if( thePlayer.GetInventory().IsQuickSlotItem( currItem ) )
			{
				//quick items don't have ammo and should be displayed
				updateItem = true;
			}
			//modW3ReduxRGC--
			else
	...
	//modW3ReduxRGC++
	public function RGC_ActOnBackupBomb()
	{
		GetWitcherPlayer().RGC_SelectItem(EES_Petard1);
	}
	
	public function RGC_ActOnBackupPotion(currentPotion : SItemUniqueId)
	{
		var backupPotion : SItemUniqueId;
		var slotUpperPotionUniqueId : SItemUniqueId;
		var slotLowerPotionUniqueId : SItemUniqueId;
		
		if(GetWitcherPlayer().GetItemEquippedOnSlot(GetWitcherPlayer().GetSelectedPotionSlotUpper(), slotUpperPotionUniqueId))
		{
			if(slotUpperPotionUniqueId == currentPotion)
			{
				GetWitcherPlayer().FlipSelectedPotion(true);
			}
		}
		
		if(GetWitcherPlayer().GetItemEquippedOnSlot(GetWitcherPlayer().GetSelectedPotionSlotLower(), slotLowerPotionUniqueId))
		{
			if(slotLowerPotionUniqueId == currentPotion)
			{
