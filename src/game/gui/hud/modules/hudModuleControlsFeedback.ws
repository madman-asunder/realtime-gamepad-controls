...
{		
	...
	//modW3ReduxRGC++
	private var m_currentInputModifier	: bool;
	private var m_previousInputModifier	: bool;
		default m_previousInputModifier = false;
	//modW3ReduxRGC--
	...
	event OnTick( timeDelta : float )
	{
		...
		//modW3ReduxRGC++
		m_currentInputModifier = GetWitcherPlayer().RGC_IsInRealtimeEquipCastMode();
		//modW3ReduxRGC--
		...
		//modW3ReduxRGC++
		else if( (m_currentInputContext == 'Combat' || m_currentInputContext == 'Exploration') && m_previousInputModifier != m_currentInputModifier )
		{
	...
	private function SendInputContextActions( inputContextName :name, optional isForced : bool )
	{
		...
		if( GetEnabled() )
		{
			...
			switch(inputContextName)
			{
				...
				case 'Exploration' :
					//modW3ReduxRGC++
					if(m_currentInputModifier)
					{
						//sign modifier controls
						l_ActionsArray.PushBack('RGC_CastIgni');
						l_ActionsArray.PushBack('RGC_CastAard');
						l_ActionsArray.PushBack('RGC_CastQuen');
						l_ActionsArray.PushBack('RGC_CastYrden');
					}
					else
					{
						...
						if( !thePlayer.IsCiri() )
						{
							...
							if( m_displayCallHorse )
							{
								...
								{
									l_ActionsArray.PushBack('RGC_SpawnHorse');
								}
								else
								{
				...
				case 'Combat' : 
					...
					else if(m_currentInputModifier)
					{
						//sign modifier controls
						l_ActionsArray.PushBack('RGC_CastIgni');
						l_ActionsArray.PushBack('RGC_CastAard');
					...
					{
						//standard combat controls
						l_ActionsArray.PushBack('AttackLight');
						l_ActionsArray.PushBack('AttackHeavy');
						l_ActionsArray.PushBack('Dodge');
						l_ActionsArray.PushBack('RGC_RealtimeModifier');
					}
					break;
			...
			for( i = 0; i < l_ActionsArray.Size(); i += 1 )
			{
				...
				switch (curAction)
				{
					...
					case 'Sprint':
					case 'SprintToggle':
						if ( theInput.IsToggleSprintBound() || RGCConfigIsImmersiveMotionEnabled() || thePlayer.GetLeftStickSprint())
				...
				else if( (curAction == 'Sprint' || curAction == 'SprintToggle' || curAction == 'RGC_Sprint') )
				{
					if ( m_currentInputContext == 'Swimming' || m_currentInputContext == 'Diving')
					{
