...
{	
	...
	event OnForceHolster( optional forceUpperBodyAnim, instant, dropItem : bool ) 
	{
		...
		//modW3ReduxRGC++
		theInput.ForceDeactivateAction('RGC_CrossBow');
		theInput.ForceDeactivateAction('RGC_CrossBowHold');
		//modW3ReduxRGC--
...
class Crossbow extends RangedWeapon
{
	...
	event OnReplaceAmmo()
	{
			...
			shotCount = shotCountLimit + 1; //modW3ReduxRGC
			
			//modW3ReduxRGC++
			ReloadWeaponWithOrWithoutAnimIfNeeded();
			RaiseOwnerGraphEvents( 'Crossbow_Reload', true );
			//modW3ReduxRGC--
...
state State_WeaponAim in RangedWeapon
{
	...
	{	
			...
			&& parent.ownerPlayer.GetBehaviorVariable( 'isAimingWeapon' ) == 0.f )
		{
			...
			//modW3ReduxRGC++
			else if ( theInput.GetActionValue( 'RGC_CrossBow' ) == 0.f )
				parent.AddTimer( 'HolsterAfterDelay', 0.f );
			//modW3ReduxRGC--
		}
		else
	...
	entry function CheckGotoAimThrow()
	{
		...
		//modW3ReduxRGC++
		if ( theInput.GetActionValue( 'RGC_CrossBow' ) == 1.f 
			|| theInput.GetActionValue( 'RGC_HorseCrossBow' ) == 1.f )
		{
		//modW3ReduxRGC--	
